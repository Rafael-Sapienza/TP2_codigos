...
fn print_most_frequent_words(number_of_words: u64, vector_word_freq: Vec<(String, u64)>) {
    ...
    for i in 0..number_of_words {
        println!(
            "palavra: {}, frequência: {}",
            vector_word_freq[i as usize].0, vector_word_freq[i as usize].1
        );
    }
}
...
fn main() {
    ...
    let stop_words_vector: Vec<String> =
        stop_words_file_to_vector("../../input_exemplo_1/stop_words.txt".to_string());
